...
class GroupRepository extends BaseRepository
{
    ...
    protected $fieldSearchable = [
        'name', 'description', 'photo_url', 'group_type', 'privacy', 'number',
    ];
    ...
    public function addMembersToGroupInRegister($group, $users, $isAdmin)
    {
        $groupUsers = $group->users->pluck('id')->toArray();
        $newAddedUsers = [];
        $newUserNames = '';
        ...
        foreach ($users as $userId) {
            ...
            GroupUser::create([
                'user_id' => $user->id,
                'group_id' => $group->id,
                'added_by' => 1, // by default added_by as "super admin" then update this value by approving user
                'role' => $isAdmin ? GroupUser::ROLE_ADMIN : GroupUser::ROLE_MEMBER,
            ]);
        }
     ...
     */
    public function getAvailableCenters($user)
    {
    ...
    public function getNextNumber($center_id)
    {
        $maxNumber = Group::where('center_id', $center_id)->max('number');
