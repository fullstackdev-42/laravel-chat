...
class UserRepository extends BaseRepository
{
    ...
    protected $fieldSearchable = [
        'name', 'phone', 'last_seen', 'about', 'photo_url', 'is_online',
    ];
    ...
    public function activeDeActiveUser($id)
    {
        ...
        if($user->is_active == false) {
            if($user->role_name != 'Center Admin') {
                // when activate the user, need to assign "added_by to group" id should be current user id
                $groupUsers = GroupUser::whereUserId($user->id)->update(['added_by' => getLoggedInUserId()]);
            }
        }
    ...
    {
        $maxUserNo = User::where('user_no', 'like', "$prefix%")->max('user_no');
        $lastIncrementalNumber = $maxUserNo ? (int) substr(strrchr($maxUserNo, '_'), 1) : 0;
        return $lastIncrementalNumber + 1;
    }


    public function assignCenters($user, $input)
    {
        $centers = ! empty($input['center_id']) ? $input['center_id'] : [];
        /** @var User $user */
